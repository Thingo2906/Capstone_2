...
class User {
  ...
  static async register({ username, password, email, isAdmin }) {
    ...
    const result = await db.query(
      `INSERT INTO users (username, password, email, is_admin) VALUES ($1, $2, $3, $4) RETURNING username, email, is_admin AS "isAdmin"`,
   ...
   *
   * Returns { username, email, is_admin, movieList }
  ...
  static async get(username) {
    const result = await db.query(
      `SELECT username, email, is_admin AS "isAdmin" FROM users WHERE username = $1`,
      [username]
    );
    const user = result.rows[0];
  ...
  static async addToList(username, movie_name, movie_id) {
    ...
    if (!user) throw new NotFoundError(`No user: ${username}`);
    await db.query(
      `INSERT INTO movies(username, movie_name, movie_id) VALUES ($1, $2, $3)`,
  ...
  static async getMoviesByUsername(username) {
    ...
    const movies = await db.query(`SELECT movie_id, movie_name FROM movies WHERE username = $1`, [
      username]);
    console.log(movies);
    return movies.rows;
  }
